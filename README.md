...
However, the communication is not atomic (nor idempotent). [The ReportingTask's event consumer updates its state after the events are transmitted](https://github.com/apache/nifi/blob/6f958fe068d67e96f93830d3244c57dda753f9d2/nifi-extension-bundles/nifi-extension-utils/nifi-reporting-utils/src/main/java/org/apache/nifi/reporting/util/provenance/ProvenanceEventConsumer.java#L217-222), therefore they may be duplicated if the cluster restarts. Additionally, [the update of the provenance repository occurs non-atomically before that of the FlowFile repository](https://github.com/apache/nifi/blob/64f5b8ee6256eeec5843ab59d95efb73d8a690f2/nifi-framework-bundle/nifi-framework/nifi-framework-components/src/main/java/org/apache/nifi/controller/repository/StandardProcessSession.java#L590), meaning that if the cluster is restarted between the two, the message will be duplicated in the provenance repository but not in the FlowFile repository, which is inconsistent.

Though, one can work around these limitations by specifying that one message was effectively duplicated in the output if:
- It is written to multiple distinct files (i.e. same topic/partition/offset combination with multiple distinct filenames)
- Or, it is written multiple times in the same file **as a part of the same event** (i.e. same topic/partition/offset combination multiple times in the same **row**)
...
Well, actually, one could achieve exactly-once ingestions via end-to-end idempotence, i.e. by designing a solution such that a same message could be ingested multiple times without side effect. This could be achieved, for example, by overwriting with a filename based only on the Kafka provenance attribute. However, the merger of messages makes it impossible to provide such guarantee, as two FlowFiles duplicating the same message could very well be merged together. And, data ingestion without merger may be unsuitable for most applications, as [HDFS is designed for large objects](https://hadoop.apache.org/docs/stable/hadoop-project-dist/hadoop-hdfs/HdfsDesign.html#Data_Blocks), and alternative compatible cloud storage services include a billing per stored object (e.g. Azure [ADLS](https://azure.microsoft.com/en-us/pricing/details/storage/data-lake/), or Amazon [EMRFS](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-fs.html) on top of [S3](https://aws.amazon.com/s3/pricing/)).

Finally, we discussed a few monitoring systems for anomaly detection, based on the Kafka provenance of the ingested messages. Embedding a monitoring system within the ingestion flow guarantees reliability, however it adds much bloat and may degrade the performance. As an alternative, one can host and share a monitoring process group on another NiFi instance, which requires to be extra careful in the anomaly detection process, in order not to be vulnerable to the duplications it is itself subject to.
